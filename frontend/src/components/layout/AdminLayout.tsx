  ...
  return (
    <div className="min-h-screen bg-surface flex">
      ...
      <motion.aside
        ...
        className="hidden md:flex flex-col bg-surface-50 border-r border-white/5 flex-shrink-0 relative sticky top-0 h-screen overflow-y-auto"
      >
        <button
  onClick={() => setSidebarOpen(!sidebarOpen)}
  className="absolute -right-3.5 top-7 w-7 h-7 bg-surface-100 border border-brand-500/50 rounded-full flex items-center justify-center hover:bg-brand-500 hover:border-brand-500 transition-all duration-200 z-20 shadow-glow-sm group"
>
          <ChevronRight
  size={13}
  className={`transition-transform duration-300 text-black font-bold ${sidebarOpen ? "rotate-180" : ""}`}
/>
        </button>
        <SidebarContent />
      </motion.aside>
